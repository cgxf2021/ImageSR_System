    ...
    <div class="outer-box" ref="outerBoxRef">
        ...
        <div class="page-header">
            <el-page-header @back="onBack">
                <template #content>
                    <span class="text-large font-600 mr-3"> 基于SRCNN模型的图像超分辨率重建 </span>
                </template>
                <div class="mt-4 text-sm font-bold">
                    基于SRCNN模型构建的图像超分辨率重建网页，点击下方( + )上传需要重建的图片; <br />
            ...
            <el-upload ref="uploadRef" v-model:file-list="fileList" action="#" list-type="picture-card"
                ...
                <template #tip>
                    <div class="el-upload__tip">
                        支持上传jpeg/png类型的图片文件，图片文件大小需要小于500kb <br />
                        最多上传3张图片
                    </div>
                </template>
            </el-upload>
...
const submitUpload = () => {
    ...
    if (fileList.value.length === 0) {
        ...
        const badImageSizeIndex = judge.searchErrorImageSize(fileList, 500)
        ...
        if (badImageTypeIndex.length === 0 && badImageSizeIndex.length === 0 && submitOnceSign.value == true) {
            ...
            if (badImageSizeIndex.length > 0) {
                ElMessage({
                    message: `这些图片大小超过500kb: ${badImageSizeIndex}`,
                    grouping: true,
                    type: 'info',
                    showClose: true
                })
